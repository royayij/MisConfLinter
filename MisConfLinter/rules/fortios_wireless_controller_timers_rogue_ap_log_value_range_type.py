...
class ValueRangeTypeRule(AnsibleLintRule):
    ...
    description = 'Value  must be in valid Range. See https://docs.ansible.com/ansible/latest/collections/fortinet/fortios/fortios_wireless_controller_timers_module.html for more details'
    tags = ['Illegal Value']
    param_value_ranges ={'between': (0, 1440)}

    param_type = int

    _modules = ['fortios_wireless_controller_timers']
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"].split('.')[-1] in self._modules:
            value = task['action'].get('rogue_ap_log', None)
