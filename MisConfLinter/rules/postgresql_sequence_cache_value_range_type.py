...
class ValueRangeTypeRule(AnsibleLintRule):
    ...
    description = 'Value  must be in valid Range. See https://docs.ansible.com/ansible/latest/collections/community/postgresql/postgresql_sequence_module.html for more details'
    tags = ['Illegal Value']
    param_value_ranges ={'min': 1}

    param_type = int

    _modules = ['postgresql_sequence']
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"].split('.')[-1] in self._modules:
            value = task['action'].get('cache', None)
