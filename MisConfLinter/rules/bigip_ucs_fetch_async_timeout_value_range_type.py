...
class ValueRangeTypeRule(AnsibleLintRule):
    ...
    description = 'Value  must be in valid Range. See https://docs.ansible.com/ansible/latest/collections/f5networks/f5_modules/bigip_ucs_fetch_module.html for more details'
    tags = ['Illegal Value']
    param_value_ranges ={'between': (150, 1800)}

    param_type = int

    _modules = ['bigip_ucs_fetch']
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"].split('.')[-1] in self._modules:
            value = task['action'].get('async_timeout', None)
