...
class ValueRangeTypeRule(AnsibleLintRule):
    ...
    description = 'Value  must be in valid Range. See https://docs.ansible.com/ansible/latest/collections/f5networks/f5_modules/bigip_monitor_smtp_module.html for more details'
    tags = ['Illegal Value']
    param_value_ranges ={'between': (1, 65535)}

    param_type = str

    _modules = ['bigip_monitor_smtp']
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"].split('.')[-1] in self._modules:
            value = task['action'].get('port', None)
