...
class ValueRangeTypeRule(AnsibleLintRule):
    ...
    description = 'Value  must be in valid Range. See https://docs.ansible.com/ansible/latest/collections/cisco/ucs/ucs_storage_profile_module.html for more details'
    tags = ['Illegal Value']
    param_value_ranges ={'between': (1, 10240)}

    param_type = str

    _modules = ['ucs_storage_profile']
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"].split('.')[-1] in self._modules:
            value = task['action'].get('size', None)
