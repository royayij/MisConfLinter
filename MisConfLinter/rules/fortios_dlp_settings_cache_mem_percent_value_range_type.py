...
class ValueRangeTypeRule(AnsibleLintRule):
    ...
    description = 'Value  must be in valid Range. See https://docs.ansible.com/ansible/latest/collections/fortinet/fortios/fortios_dlp_settings_module.html for more details'
    tags = ['Illegal Value']
    param_value_ranges ={'between': (1, 15)}

    param_type = int

    _modules = ['fortios_dlp_settings']
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"].split('.')[-1] in self._modules:
            value = task['action'].get('cache_mem_percent', None)
