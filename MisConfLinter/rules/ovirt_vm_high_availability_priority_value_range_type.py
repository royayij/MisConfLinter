...
class ValueRangeTypeRule(AnsibleLintRule):
    ...
    description = 'Value  must be in valid Range. See https://docs.ansible.com/ansible/latest/collections/ovirt/ovirt/ovirt_vm_module.html for more details'
    tags = ['Illegal Value']
    param_value_ranges ={'between': (0, 100)}
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"].split('.')[-1] in self._modules:
            value = task['action'].get('high_availability_priority', None)
