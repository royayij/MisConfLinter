...
class ValueRangeTypeRule(AnsibleLintRule):
    ...
    description = 'Value  must be in valid Range. See https://docs.ansible.com/ansible/latest/collections/community/vmware/vmware_vspan_session_module.html for more details'
    tags = ['Illegal Value']
    param_value_ranges ={'between': (1, 65535)}

    param_type = int

    _modules = ['vmware_vspan_session']
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"].split('.')[-1] in self._modules:
            value = task['action'].get('sampling_rate', None)
