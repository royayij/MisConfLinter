...
class ValueRangeTypeRule(AnsibleLintRule):
    ...
    description = 'Value  must be in valid Range. See https://docs.ansible.com/ansible/latest/collections/openstack/cloud/lb_member_module.html for more details'
    tags = ['Illegal Value']
    param_value_ranges ={'between': (0, 256)}
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"].split('.')[-1] in self._modules:
            value = task['action'].get('weight', None)
