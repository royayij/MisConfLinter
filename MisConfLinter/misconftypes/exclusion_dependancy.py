...
def inclusion_dep_def(data, item, module_name, param_name):
    if not re.search(r'(ignored|removed|not need)', item):
        try:
            ...
            boolean_choices = ['no', 'enabled', 'set', 'is specified', 'default', '*', '+', '=']
            ...
            if re.search(r'({})'.format(pattern_params), item):
                if re.search(r'\b(if|when|unless)\b', item) and re.search(r'\b(even|not|no)\b', item) and bfs_tree.count('SBAR') == 1:
                    try:
...
def detect_conditional_exclusion(data, item):
    if not re.search(r'(ignored|removed|not need)', item[2]):
        det_result = {item[1]: []}
        tree = tree_sentence(parser, item[2])
        bfs_tree = bfs_traversal(tree)
        boolean_choices = ['no', 'not enabled', 'not set', 'is not specified', '*', '+', 'default']
        ...
        for key in det_result.keys():
            ...
            if re.search(r'\b({})\b'.format('|'.join(['not enabled', 'not set', 'is not specified', 'no'])),
                         ' '.join(det_result[key])):
                det_result[key] = 'True'

        return det_result
def gen_rule_exclusion_dependency_type(df_parameter_level_texts, env):
    ...
    for each_val in sel_mod_parameters:
        exc_dep_each_val = detect_conditional_exclusion(df_parameter_level_texts, each_val)
        rule_name = 'ExclusionDependencyTypeRule'
        tm = env.get_template('exclusion_dependency_type_template.txt')
        python_render_file = tm.render(rule_name=rule_name, rule_id=f'exclusion_dependency_type_{each_val[0]}_{each_val[1]}',
                                       ...
                                       exc_dep_dict=exc_dep_each_val,
                                       misconfiguration_type='Illegal Value')
        # to save the results
        with open("rules/" + each_val[0] + "_" + each_val[1] + "_exclusion_dependency_type.py", "w") as fh:
